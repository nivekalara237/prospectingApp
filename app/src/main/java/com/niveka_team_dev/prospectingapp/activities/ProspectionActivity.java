...
public class ProspectionActivity extends AppCompatActivity implements RadioGroup.OnCheckedChangeListener {
    ...
    private void attemptSave() {
        ...
        if (TextUtils.isEmpty(nom)){
            cancel = true;
            focus = name;
            errors.add("Nom Client obligatoire");
        }

        if (TextUtils.isEmpty(loc)){
            cancel = true;
            focus = location;
            errors.add("Localisation obligatoire");
        }
        if (TextUtils.isEmpty(tel) && TextUtils.isEmpty(em)){

            cancel = true;
            focus = phone;
            errors.add("Telephone ou email obligatoire");
            Toasty.success(this, "Vous devez renseigner l'email et le numero de telephone, ou bien l'un des deux", Toast.LENGTH_SHORT,false).show();
        }
